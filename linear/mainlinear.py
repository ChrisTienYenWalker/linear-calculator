...
while  2 > 1:
    linearQues = input('Would you like to find [slope, rate, x-int, y-int, formula, descprition, prediction] ')
    linearQues.lower()
    linearQues.strip()
    if linearQues == 'exit':
        break
    ...
    if linedec == 'formula':
        ...
        if linearQues == 'prediction':
            ...
            if predict == 'x':
                ...
                print('X is ' + formula.predictX(0, m, b, y))
            if predict == 'y':
                x = int(input('What is the value of X: '))
                print('Y is ' + formula.predictY(0, m, b, x))
